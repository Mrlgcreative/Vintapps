...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <form action="{{ route('vendeur.annonces.update', $annonce) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf @method('PATCH')
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <select name="devise" id="devise" required
                            ...
                            <option value="USD" {{ old('devise', $annonce->devise) === 'USD' ? 'selected' : '' }}>$</option>
                            <option value="CDF" {{ old('devise', $annonce->devise) === 'CDF' ? 'selected' : '' }}>FC</option>
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-5 gap-2">
                    @foreach(['neuf' => 'Neuf', 'tres_bon_etat' => 'Très bon état', 'bon_etat' => 'Bon état', 'usage' => 'Usage', 'endommage' => 'Endommagé'] as $val => $label)
                        <label class="flex items-center gap-2 px-3 py-2.5 border border-gray-300 rounded-lg cursor-pointer has-[:checked]:border-[#723EC3] has-[:checked]:bg-[#723EC3]/5 transition">
                            <input type="radio" name="etat" value="{{ $val }}" {{ old('etat', $annonce->etat) === $val ? 'checked' : '' }} required
        ...
        @if($annonce->photos->isNotEmpty())
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <span class="block text-sm font-medium text-gray-700 mb-3">Photos actuelles</span>
                <div class="grid grid-cols-5 gap-2">
                    @foreach($annonce->photos as $photo)
                        <div class="relative aspect-square rounded-lg overflow-hidden bg-gray-100 border border-gray-200 group">
                            <img src="{{ Storage::url($photo->chemin_fichier) }}" alt="" class="w-full h-full object-cover">
                            <a href="{{ route('vendeur.annonces.photo.destroy', $photo) }}"
                               onclick="return confirm('Supprimer cette photo ?')"
                               class="absolute top-1 right-1 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 hover:bg-red-600 transition">
                                &times;
                            </a>
                        </div>
                    @endforeach
        ...
        {{-- Nouvelles photos --}}
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <span class="block text-sm font-medium text-gray-700 mb-1">Ajouter des photos</span>
            <p class="text-xs text-gray-400 mb-4">Formats : JPG, PNG, WEBP. Max 5 Mo.</p>
